...
impl RetrieveParams {
    ...
    fn interval_adjust_ts_start(&self, ts: i64, interval_ns: i64) -> Result<i64> {
        if ts % interval_ns == 0 {
    ...
    fn interval_adjust_ts_end(&self, ts: i64, interval_ns: i64) -> Result<i64> {
        if ts % interval_ns == 0 {
            // If the timestamp is already aligned to the interval, return it as is
            Ok(ts)
        } else {
            // If not aligned, round up to the next interval boundary
            Ok(ts + (interval_ns - (ts % interval_ns)))
        }
    }

    fn batch_interval(&mut self, interval_ns: i64, batch_size: i64) -> Result<i64> {
        // Adjust start timestamp
        self.start_ts = self.interval_adjust_ts_start(self.start_ts, interval_ns)?;
        ...
        if calculated_end_ts > self.end_ts {
            let end = self.interval_adjust_ts_end(self.end_ts, interval_ns)?;
            Ok(end)
...
//             params.schema, params.symbols, params.start_ts, end_ts
//         );
//
...
//
//         info!("Successfully queried, parsing records to objects.");
//
//         for row in rows {
...
impl RecordsQuery for Mbp1Msg {
    async fn retrieve_query(
        ...
        params: &mut RetrieveParams,
        batch: i64,
    ) -> Result<impl futures::Stream<Item = Result<sqlx::postgres::PgRow>>> {
        // Batch timestamps
        let interval_ns = params.schema_interval()?;
        let end_ts = params.batch_interval(interval_ns, batch)?;
