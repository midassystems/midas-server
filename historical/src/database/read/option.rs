pub const OPTION_MBP1_QUERY: &str = r#"
            ...
            FROM option_mbp m
            INNER JOIN option i ON m.instrument_id = i.instrument_id
            LEFT JOIN option_bid_ask b ON m.id = b.mbp_id AND b.depth = 0
            WHERE m.ts_recv BETWEEN $1 AND $2
            AND i.ticker = ANY($3)
            AND ($4 IS FALSE OR m.action = 84)
            "#;

pub const OPTION_TRADE_QUERY: &str = r#"
...
pub const OPTION_OHLCV_QUERY: &str = r#"
        ...
        INNER JOIN option i ON a.instrument_id = i.instrument_id
        ORDER BY a.ts_event
        "#;

pub const OPTION_BBO_QUERY: &str = r#"
            WITH ordered_data AS (
                SELECT
                    m.id,
                    m.instrument_id,
            ...
            INNER JOIN option i ON fp.instrument_id = i.instrument_id
