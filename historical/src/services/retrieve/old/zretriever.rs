...
//     pub continuous_map: HashMap<String, i32>,
// }
...
//         for (kind, rank_map) in self.continuous_map.type_map.iter() {
...
//                     &self.pool, start, end, rank, tickers, &kind,
//                 )
//                 .await?;
//             }
//         }
...
pub struct RecordGetter {
    ...
    id_map: HashMap<i32, HashMap<String, u32>>,
    type_map: HashMap<ContinuousKind, HashMap<i32, Vec<String>>>,
    continuous_ticker_map: HashMap<String, Vec<RollingWindow>>,
}
...
impl RecordGetter {
    pub async fn new(batch_size: i64, params: RetrieveParams, pool: PgPool) -> Result<Self> {
        ...
        Ok(RecordGetter {
            ...
            type_map: HashMap::new(),
            id_map: HashMap::new(),
            continuous_ticker_map: HashMap::new(),
        })
    }
    ...
    pub async fn build_continuous_map(&mut self, start: i64, end: i64) -> Result<()> {
        for (kind, rank_map) in self.type_map.iter() {
            for (rank, tickers) in rank_map {
                info!(
                    "Processing kind: {}, rank: {}, tickers: {:?}",
                    kind, rank, tickers
                ...
                let map = RollingWindow::retrieve_continuous_window(
                    &self.pool, start, end, *rank, symbols, &kind,
                )
                .await?;

                self.continuous_ticker_map.extend(map);
            }
        }
        // .expect("Error on retrieve records.");
        Ok(())
    ...
    pub async fn process_continuous_records(self: Arc<Self>) -> Result<()> {
        ...
        let _ = params.interval_adjust_ts_end()?;
        // let final_end = params.end_ts;
        let start = params.start_ts;
        let end = params.end_ts;
        ...
        for (kind, rank_map) in continuous_map.type_map.iter() {
            ...
            for (rank, tickers) in rank_map {
                ...
                let vec = RollingWindow::retrieve_continuous_window(&self.pool, &params, &kind)
                    .await
                    .expect("Error on retrieve records.");

                while (end - params.start_ts) > 86_400_000_000_001 {
                    params.end_ts = params.start_ts + 86_400_000_000_000;
    ...
    pub async fn process_continuous_records2(self: Arc<Self>) -> Result<()> {
        // Clone the parameters to avoid locking or mutability issues
        let mut params = self.retrieve_params.lock().await.clone();
        // let rolling_schedule: Vec<RollingWindow> = RollingWindow::
        // let rtype = RType::from(params.rtype().unwrap());
