...
pub async fn create_record(
    ...
    let loader = RecordLoader::new(1000, metadata.dataset, state.historical_pool.clone()).await?;
...
pub async fn bulk_upload(
    ...
    // Initialize the loader
    let loader = RecordLoader::new(20_000, metadata.dataset, state.historical_pool.clone()).await?;
    let progress_stream = loader.process_records(decoder).await;
...
mod test {
    ...
    use super::*;
    use crate::historical::services::retrieve::get_records;
    ...
    async fn test_bulk_upload() -> anyhow::Result<()> {
        ...
        // Test
        let result = bulk_upload(Extension(state.clone()), Json(file.to_string()))
            .await
            .into_response();
        ...
        let response = get_records(Extension(state.clone()), Json(params))
            .await
            .into_response();
