...
use crate::trading::database::live::{create_live_related, retrieve_live_related, LiveDataQueries};
use crate::Error;
...
pub async fn list_live(
    ...
    match LiveData::retrieve_list_query(&state.trading_pool).await {
        Ok(data) => {
...
pub async fn retrieve_live(
    ...
    match retrieve_live_related(&state.trading_pool, id).await {
        Ok(live) => {
...
mod test {
    ...
    #[sqlx::test]
    #[serial]
    async fn test_create_live() -> anyhow::Result<()> {
        dotenv::dotenv().ok();
        ...
        // Test
        let response = create_live(Extension(state.clone()), Json(live_data))
            .await
            .unwrap()
            .into_response();
        ...
        if number.is_some() {
            let _ = delete_live(Extension(state.clone()), Json(number.unwrap())).await;
        }
        Ok(())
    }
    ...
    #[sqlx::test]
    #[serial]
    async fn test_list_live() -> anyhow::Result<()> {
        dotenv::dotenv().ok();
        // let pool = init_db().await.unwrap();
        let state = create_db_state().await?;
        ...
        let result = create_live(Extension(state.clone()), Json(live_data))
            .await
            .unwrap()
            .into_response();
        ...
        // Test
        let response = list_live(Extension(state.clone()))
            .await
            .unwrap()
            .into_response();
    ...
    #[sqlx::test]
    #[serial]
    async fn test_retrieve_live() -> anyhow::Result<()> {
        dotenv::dotenv().ok();
        // let pool = init_db().await.unwrap();
        let state = create_db_state().await?;
        ...
        let result = create_live(Extension(state.clone()), Json(live_data_obj.clone()))
            .await
            .unwrap()
            .into_response();
        ...
        // Test
        let response = retrieve_live(Extension(state.clone()), Query(params))
            .await
            .unwrap()
            .into_response();
    ...
    #[sqlx::test]
    #[serial]
    async fn test_delete_live() -> anyhow::Result<()> {
        dotenv::dotenv().ok();
        // let pool = init_db().await.unwrap();
        let state = create_db_state().await?;

        // Test
        let result = delete_live(Extension(state.clone()), Json(63))
            .await
            .unwrap()
            .into_response();
